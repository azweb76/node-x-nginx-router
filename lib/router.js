...
function stop(cb){
    ...
    if (currentContext.processes){
	    for(proc in currentContext.processes){
	    	console.log('stopping ' + proc);
	    	var p = currentContext.processes[proc];
	    	if (p.respawnTimeout) { clearTimeout(p.respawnTimeout); }
	    	if (p.okTimeout) { clearTimeout(p.okTimeout); }
	    	if (p.lastPid) { p.process.kill('SIGINT'); }
	    }
	}
